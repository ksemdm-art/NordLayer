...
import { computed, onMounted, onUnmounted, ref, nextTick, h, watch } from 'vue'
...
const socialButtons = computed(() => [
  {
    name: 'Twitter',
    icon: () => h('svg', { class: 'w-4 h-4', fill: 'currentColor', viewBox: '0 0 24 24' }, [
  ...
  },
  {
    ...
    url: (title: string, url: string) => `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(url)}`
  },
  {
    name: 'LinkedIn',
    icon: () => h('svg', { class: 'w-4 h-4', fill: 'currentColor', viewBox: '0 0 24 24' }, [
    ...
    url: (title: string, url: string) => `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(url)}`
  },
  {
    name: 'Telegram',
    icon: () => h('svg', { class: 'w-4 h-4', fill: 'currentColor', viewBox: '0 0 24 24' }, [
    ...
    url: (title: string, url: string) => `https://t.me/share/url?url=${encodeURIComponent(url)}&text=${encodeURIComponent(title)}`
  }
])
...
const shareToSocial = (social: any) => {
  ...
  const shareUrl = social.url(title, url)
